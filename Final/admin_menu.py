...
class AdminNotificationsMenu:
    def __init__(self, root):
        ...
        self.company_name_font = ("Segoe UI Semibold", 11)
        self.notification_font = ("Segoe UI Semibold", 11)
        self.entry_font = ("Arial", 10)

    def display(self):
        company_label = ttk.Label(self.root, text="Choose Bus Company Name:", font=self.company_name_font)
        company_label.place(relx=0.5, rely=0.28, anchor="center")

        company_combobox = ttk.Combobox(self.root, textvariable=self.company_name_var, values=["JOYBUS", "FARIÑAS", "PARTAS"], font=self.entry_font)
        company_combobox.place(relx=0.5, rely=0.33, anchor="center")

        notification_label = ttk.Label(self.root, text="Enter Notification:", font=self.notification_font)
        notification_label.place(relx=0.5, rely=0.38, anchor="center")

        notification_entry = tk.Text(self.root, height=7, width=35, wrap=tk.WORD, font=self.entry_font, padx=5, pady=5)
        notification_entry.place(relx=0.5, rely=0.51, anchor="center")

        notify_button = ttk.Button(self.root, text="Notify Users", command=self.notify_users)
        notify_button.place(relx=0.5, rely=0.66, anchor="center")
...
def create_admin_menu():
    ...
    sidebar_contents = [("Edit Bus Schedule", AdminScheduleMenu, "Segoe UI Semibold", 10, "#000000"),
                        ("Claim Tickets", TicketsMenu, "Segoe UI Semibold", 10, "#000000"),
                        ("Create Notifications", AdminNotificationsMenu, "Segoe UI Semibold", 10, "#000000"),
                        ("See History", HistoryMenu, "Segoe UI Semibold", 10, "#000000"),
